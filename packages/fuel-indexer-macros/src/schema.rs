    ...
    match typ {
        TypeDefinition::Object(obj) => {
            ...
            for field in &obj.fields {
                ...
                let (is_nullable, mut column_type_name) = get_column_type(type_name.clone());
...
fn process_definition(
    ...
    match definition {
        Definition::TypeDefinition(def) => process_type_def(
            query_root, namespace, types, def, processed, primitives, types_map, is_native,
        ),
...
pub(crate) fn process_graphql_schema(
    ...
    let mut file = match File::open(&path) {
        Ok(f) => f,
        Err(e) => {
            proc_macro_error::abort_call_site!("Could not open schema file {:?} {:?}", path, e)
        }
    };
