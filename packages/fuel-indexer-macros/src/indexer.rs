                    ...
                    match inp {
                        ...
                        FnArg::Typed(PatType { ty, .. }) => {
                            if let Type::Path(path) = &**ty {
                                ...
                                if DISALLOWED_ABI_JSON_TYPES.contains(path_ident_str.as_str()) {
                                    proc_macro_error::abort_call_site!(
                                ...
                                if !abi_types.contains(ty_id) {
                                    if FUEL_PRIMITIVES.contains(path_ident_str.as_str()) {
                                        ...
                                        match path_ident_str.as_str() {
                                            ...
                                            "Log" => {
                                                log_decoder = quote! { self.#name.push(data); };
                                            }
                                            "LogData" => {
                                                abi_decoders
                                                    .push(decode_snippet(*ty_id, &ty, &name));
                                            }
                                            "MessageOut" => {
                                                messageout_decoder =
                                                    quote! { self.#name.push(data); };
                                            ...
                                            "Return" => {
                                                return_decoder = quote! { self.#name.push(data); };
                                            }
                                            "ScriptResult" => {
...
pub fn get_abi_tokens(namespace: &str, abi: &String, is_native: bool) -> proc_macro2::TokenStream {
...
pub fn process_indexer_module(attrs: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let graphql_tokens =
        process_graphql_schema(namespace, identifier, schema_string, manifest.is_native());
