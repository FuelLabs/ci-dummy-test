use crate::utils::{
    ...
    get_index_directive, get_join_directive_info, get_unique_directive, normalize_field_type_name,
    BASE_SCHEMA,
};
...
impl SchemaBuilder {
    pub fn new(namespace: &str, identifier: &str, version: &str, db_type: DbType) -> SchemaBuilder {
    ...
    fn generate_table_sql(
        ...
        match typ {
            TypeDefinition::Object(o) => {
                ...
                let columns = self.generate_columns(o, type_id, &o.fields, &table_name, types_map);
...
impl Schema {
    pub async fn load_from_db(
        ...
        let typeids =
            queries::type_id_list_by_name(&mut conn, &root.schema_name, &root.version, identifier)
                .await?;
...
mod tests {
    ...
    "#;
        ...
        assert_eq!(
            ...
            "CREATE INDEX payee_hash_idx ON namespace_index1.payee USING btree (hash);".to_string()
        );
    }
