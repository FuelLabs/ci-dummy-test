...
use fuel_indexer_types::{
    Address, AssetId, Blob, Bytes32, Bytes4, Bytes8, ContractId, Identity, Int16, Int4, Int8, Json,
    MessageId, Salt, UInt16, UInt4, UInt8,
};
...
impl FtColumn {
    pub fn new(ty: ColumnType, size: usize, bytes: &[u8]) -> FtColumn {
        match ty {
            ...
            ColumnType::Int16 => {
                let int16 =
                    i128::from_le_bytes(bytes[..size].try_into().expect("Invalid slice length"));
            ...
            ColumnType::UInt16 => {
                let uint16 =
                    u128::from_le_bytes(bytes[..size].try_into().expect("Invalid slice length"));
            ...
            ColumnType::MessageId => {
                let message_id = MessageId::try_from(&bytes[..size]).expect("Invalid slice length");
            ...
            ColumnType::Identity => {
                let identity = Identity::try_from(&bytes[..size]).expect("Invalid slice length");
                FtColumn::Identity(Some(identity))
            }
            ColumnType::Boolean => {
                let value =
                    u8::from_le_bytes(bytes[..size].try_into().expect("Invalid slice length"));
...
mod tests {
    ...
    fn test_fragments_some_types() {
        ...
        let addr = FtColumn::Address(Some(Address::try_from([0x12; 32]).expect("Bad bytes")));
        let asset_id = FtColumn::AssetId(Some(AssetId::try_from([0xA5; 32]).expect("Bad bytes")));
        let bytes4 = FtColumn::Bytes4(Some(Bytes4::try_from([0xF0; 4]).expect("Bad bytes")));
        let bytes8 = FtColumn::Bytes8(Some(Bytes8::try_from([0x9D; 8]).expect("Bad bytes")));
        let bytes32 = FtColumn::Bytes32(Some(Bytes32::try_from([0xEE; 32]).expect("Bad bytes")));
        let contractid =
            FtColumn::ContractId(Some(ContractId::try_from([0x78; 32]).expect("Bad bytes")));
        ...
        let message_id =
            FtColumn::MessageId(Some(MessageId::try_from([0x0F; 32]).expect("Bad bytes")));
