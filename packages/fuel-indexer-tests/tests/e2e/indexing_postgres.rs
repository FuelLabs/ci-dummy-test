...
async fn test_can_trigger_and_index_events_with_multiple_args_in_index_handler_postgres() {
    ...
    let block_row =
        sqlx::query("SELECT * FROM fuel_indexer_test_index1.block ORDER BY height DESC LIMIT 1")
            .fetch_one(&mut conn)
            .await
            .unwrap();
    ...
    let pung_row = sqlx::query("SELECT * FROM fuel_indexer_test_index1.pungentity WHERE id = 123")
        .fetch_one(&mut conn)
        .await
        .unwrap();
    ...
    let contract_buff =
        <[u8; 32]>::from_hex("322ee5fb2cabec472409eb5f9b42b59644edb7bf9943eda9c2e3947305ed5e96")
            .unwrap();
...
async fn test_can_trigger_and_index_callreturn_postgres() {
    ...
    let mut conn = pool.acquire().await.unwrap();
    let row = sqlx::query("SELECT * FROM fuel_indexer_test_index1.pungentity WHERE id = 3")
        .fetch_one(&mut conn)
        .await
        .unwrap();
    ...
    let addr_buff =
        <[u8; 32]>::from_hex("532ee5fb2cabec472409eb5f9b42b59644edb7bf9943eda9c2e3947305ed5e96")
            .unwrap();
...
async fn test_can_trigger_and_index_blocks_and_transactions_postgres() {
    ...
    let mut conn = pool.acquire().await.unwrap();
    let row = sqlx::query("SELECT * FROM fuel_indexer_test_index1.block WHERE height = 1")
        .fetch_one(&mut conn)
        .await
        .unwrap();
...
async fn test_can_trigger_and_index_ping_event_postgres() {
    ...
    let mut conn = pool.acquire().await.unwrap();
    let row = sqlx::query("SELECT * FROM fuel_indexer_test_index1.pingentity WHERE id = 1")
        .fetch_one(&mut conn)
        .await
        .unwrap();
    ...
    let mut conn = pool.acquire().await.unwrap();
    let row = sqlx::query("SELECT * FROM fuel_indexer_test_index1.u16entity WHERE id = 9999")
        .fetch_one(&mut conn)
        .await
        .unwrap();
...
async fn test_can_trigger_and_index_log_event_postgres() {
    ...
    let row = sqlx::query("SELECT * FROM fuel_indexer_test_index1.log WHERE ra = 8675309 LIMIT 1")
        .fetch_one(&mut conn)
        .await
        .unwrap();
...
async fn test_can_trigger_and_index_logdata_event_postgres() {
    ...
    let mut conn = pool.acquire().await.unwrap();
    let row = sqlx::query("SELECT * FROM fuel_indexer_test_index1.pungentity WHERE id = 1")
        .fetch_one(&mut conn)
        .await
        .unwrap();
...
async fn test_index_metadata_is_saved_when_indexer_macro_is_called_postgres() {
    ...
    let mut conn = pool.acquire().await.unwrap();
    let row = sqlx::query("SELECT * FROM fuel_indexer_test_index1.indexmetadataentity LIMIT 1")
        .fetch_one(&mut conn)
        .await
        .unwrap();
