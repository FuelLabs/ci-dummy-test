...
use fuel_indexer_tests::assets::{SIMPLE_WASM_MANIFEST, SIMPLE_WASM_SCHEMA, SIMPLE_WASM_WASM};
use fuel_indexer_tests::fixtures::{
    api_server_app_postgres, http_client, indexer_service_postgres, postgres_connection_pool,
};
...
async fn test_metrics_endpoint_returns_proper_count_of_metrics_postgres() {
    ...
    let server =
        axum::Server::bind(&GraphQLConfig::default().into()).serve(app.into_make_service());
...
async fn test_asset_upload_endpoint_properly_adds_assets_to_database_postgres() {
    ...
    let manifest_file = multipart::Part::stream(SIMPLE_WASM_MANIFEST).file_name("simple_wasm.yaml");
    let schema_file = multipart::Part::stream(SIMPLE_WASM_SCHEMA).file_name("simple_wasm.graphql");
    let wasm_file = multipart::Part::stream(SIMPLE_WASM_WASM).file_name("simple_wasm.wasm");
