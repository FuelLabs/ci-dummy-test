...
use fuel_indexer_lib::config::{DatabaseConfig, FuelNodeConfig, GraphQLConfig, IndexerConfig};
use fuels::core::parameters::StorageConfiguration;
use fuels::{
    fuels_abigen::abigen,
    prelude::{
        setup_single_asset_coins, setup_test_client, AssetId, Bech32ContractId, Config, Contract,
...
pub async fn setup_test_fuel_node(
    ...
    let mut wallet =
        WalletUnlocked::load_keystore(wallet_path, defaults::WALLET_PASSWORD, None).unwrap();
...
pub async fn connect_to_deployed_contract() -> Result<FuelIndexerTest, Box<dyn std::error::Error>> {
    ...
    let mut wallet =
        WalletUnlocked::load_keystore(wallet_path_str, defaults::WALLET_PASSWORD, None).unwrap();
...
pub mod test_web {
    ...
    pub async fn server() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let _ =
            HttpServer::new(move || app(FuelIndexerTest::new(contract_id.clone(), wallet.clone())))
