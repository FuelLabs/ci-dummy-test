...
use fuels_types::{enum_variants::EnumVariants, errors::Error as SDKError, param_types::ParamType};
...
impl From<[u8; IDENTITY_LEN]> for Identity {
    fn from(bytes: [u8; IDENTITY_LEN]) -> Self {
        match bytes.first() {
            ...
            Some(1u8) => Self::ContractId(ContractId::try_from(&bytes[1..]).expect("Failed")),
            _ => panic!("Failed"),
        }
    }
}
