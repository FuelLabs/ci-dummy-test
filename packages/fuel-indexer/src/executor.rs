...
use fuel_indexer_lib::{
    ...
    defaults::{
        DELAY_FOR_EMPTY_PAGE, DELAY_FOR_SERVICE_ERR, INDEX_FAILED_CALLS, MAX_EMPTY_BLOCK_REQUESTS,
    },
};
...
pub fn run_executor<T: 'static + Executor + Send + Sync>(
    ...
    let client = FuelClient::from_str(fuel_node_addr)
        .unwrap_or_else(|e| panic!("Unable to connect to Fuel node at '{fuel_node_addr}': {e}",));
...
// TODO: Use mutex
unsafe impl<F: Future<Output = IndexerResult<()>> + Send> Sync for NativeIndexExecutor<F> {}
unsafe impl<F: Future<Output = IndexerResult<()>> + Send> Send for NativeIndexExecutor<F> {}
    ...
    pub async fn create(
        ...
        match &exec_source {
            ...
            ExecutorSource::Registry(bytes) => {
                let executor =
                    WasmIndexExecutor::new(db_url.into(), manifest.to_owned(), bytes).await?;
                let handle = tokio::spawn(run_executor(
                    &fuel_node.to_string(),
                    executor,
                    manifest.start_block,
