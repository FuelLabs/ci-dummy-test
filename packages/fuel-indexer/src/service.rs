...
impl IndexerService {
    ...
    pub async fn register_index_from_manifest(&mut self, manifest: Manifest) -> IndexerResult<()> {
        let database_url = self.database_url.clone();
        let mut conn = self.pool.acquire().await?;
        let index =
            queries::register_index(&mut conn, &manifest.namespace, &manifest.identifier).await?;
